...
export class SubscriptionPreferenceComponent extends BaseElement {
  ...
  private renderReady() {
    ...
    const streams = responseStreams.map((stream) => html`<button type="button" class="subscription-opt" data-stream="${stream.key}" aria-pressed="${stream.isOpted}">${stream.label}</button>`);
