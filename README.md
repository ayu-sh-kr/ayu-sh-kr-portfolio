...
| Hosting configuration | Vercel SPA rewrite configuration |
...
The SPA rewrite in [vercel.json](vercel.json) ensures direct navigation to these client-side routes is served by `index.html`.
...
Deploy the generated `dist/` directory to a static host. For Vercel, the checked-in configuration rewrites all paths to the SPA entry point and caches fingerprinted `/assets/` files for one year.
