...
Attributes on an h2 use the markdown-it-attrs syntax `{#id key="value"}`:
  #id     the anchor; once published it is permanent (people link to clauses)
  scope   the audience chip — one of: Site visitors | Clients | Everyone
  group   the TOC group heading; groups are emitted in document order,
          whenever the value changes, so never reorder to suit grouping
  short   the TOC label; always shorter than the heading

Section numbers are generated by the renderer, not written here. Add new
sections at the end of their group rather than inserting in the middle.

Components: <Aside kind="note"> and <Aside kind="quote"> only. Everything
else is plain markdown; tables render as .ltable and restack on mobile.
-->
